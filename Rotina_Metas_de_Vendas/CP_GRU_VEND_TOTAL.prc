...
------------------------------------------------------------------------------------------------------------------------------------------------------------
            IF EDGGV = 'S' THEN
                ...
                DELETE FROM AD_GRUMETEMPDIA AD WHERE AD.ID = FIELD_ID;
...
----------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    LOOP
                        SELECT COUNT(*)
                        INTO AD_GRUMETEMPDIAPK
                        FROM AD_GRUMETEMPDIA AD
                        WHERE AD.ID = FIELD_ID
                        AND AD.IDMETEMP = IEMP.IDMETEMP;
                        
                        IF AD_GRUMETEMPDIAPK = 0 THEN
                        ...
                        ELSE
                            ...
                            INSERT INTO AD_GRUMETEMPDIA (ID, IDMETEMP, IDGRUMETDIA, CODGRUPOPROD, DATA, PESO, PER, VLRVENDA, META, PERGRUEMP, CODEMP) VALUES 
                            (FIELD_ID, IEMP.IDMETEMP, AD_GRUMETEMPDIAPK, IEMPDIAS.GRUPO, IEMPDIAS.FATUR, IEMPDIAS.PER, 10, IEMPDIAS.TOTAL, IEMPDIAS.META, IEMPDIAS.PERGRU, IEMPDIAS.CODEMP); 
                        END IF;
------------------------------------------------------------------------------------------------------------------------------------------------------------          
--VERIFICA SE O GRUPO PAI É O GRUPO FILHO
------------------------------------------------------------------------------------------------------------------------------------------------------------
                            SELECT MAX(ANALITICO)
                            INTO PGRUFACIL
                            FROM TGFGRU GRU
                            WHERE GRU.CODGRUPOPROD = IEMPDIAS.GRUPO;
                            --SELECT * FROM AD_METEMPGRUSUBGRU
                            --SE O GRUPO FOR ANALÍTICO.
                            
                        IF PGRUFACIL = 'S' THEN
                            SELECT COUNT(*)
                            INTO AD_METEMPGRUSUBGRUPK
                            FROM AD_METEMPGRUSUBGRU AD
                            WHERE AD.ID = FIELD_ID
                            AND AD.IDMETEMP = IEMP.IDMETEMP;
                            
                            IF AD_METEMPGRUSUBGRUPK = 0 THEN
                                AD_METEMPGRUSUBGRUPK := 1;
                                INSERT INTO AD_METEMPGRUSUBGRU (ID, IDMETEMP, IDGRUMETDIA, IDEMPGRUFI, CODGRUPOPROD, VLR, CODEMP, DATA, PESO, META) VALUES
                                (FIELD_ID, IEMP.IDMETEMP, AD_GRUMETEMPDIAPK, AD_METEMPGRUSUBGRUPK, IEMPDIAS.GRUPO, IEMPDIAS.META, IEMP.CODEMP, IEMPDIAS.FATUR, 100, IEMPDIAS.META);
                            ELSE
                            ...
                            END IF;
----------
--GRAVA OS VENDEDORES DOS GRUPOS PAI IGUAIS AOS GRUPOS FILHOS
----------
                                ...
                                LOOP
                                    SELECT COUNT(*)
                                    INTO AD_METEMPGRUSUBGRUVENPK
                                    FROM AD_METEMPGRUSUBGRUVEN AD
                                    WHERE AD.ID = FIELD_ID
                                    AND AD.IDMETEMP = IEMP.IDMETEMP
                                    AND AD.IDGRUMETDIA = AD_GRUMETEMPDIAPK
                                    ...
                                    AD_METEMPGRUSUBGRUVENPK := 1;
                                        ...
                                        (FIELD_ID, IEMP.IDMETEMP, AD_GRUMETEMPDIAPK, AD_METEMPGRUSUBGRUPK,AD_METEMPGRUSUBGRUVENPK, IGRUFIVEND.GRUPO, IGRUFIVEND.TOTAL, IEMP.CODEMP, IGRUFIVEND.FATUR, IGRUFIVEND.CODVEND, (IGRUFIVEND.TOTAL/IGRUFIVEND.TOTALZAO)*100);
...
------------------------------------------------------------------------------------------------------------------------------------------------------------
                            FOR IGRUFI IN (SELECT PRO.CODGRUPOPROD AS GRUPO
                                              , TO_CHAR(TRUNC(CAB.DTFATUR), 'd') AS DATA
                                              , TRUNC(CAB.DTFATUR) AS FATUR
                                              ------------------------------------TOTALZAO
                                              , (SELECT SUM( ((ITE.VLRTOT - ITE.VLRDESC - ITE.VLRREPRED + ITE.VLRSUBST + ITE.VLRIPI)  * VCA.INDITENSBRUTO) * CASE WHEN TOP.BONIFICACAO = 'S' THEN 0 ELSE 1 END * TOP.GOLDEV ) AS TOTAL
                            ...
                            LOOP
                                ...
                                END IF;  
                                    ...
                                    INSERT INTO AD_METEMPGRUSUBGRU (ID, IDMETEMP, IDGRUMETDIA, IDEMPGRUFI, CODGRUPOPROD, VLR, CODEMP, DATA, PESO, META) VALUES
                                    (FIELD_ID, IEMP.IDMETEMP, AD_GRUMETEMPDIAPK, AD_METEMPGRUSUBGRUPK, IGRUFI.GRUPO, IGRUFI.TOTAL, IEMP.CODEMP, IGRUFI.FATUR, (IGRUFI.TOTAL / IGRUFI.TOTALZAO)*100,(IEMPDIAS.META / 100) * (IGRUFI.TOTAL / IGRUFI.TOTALZAO)*100);
...
--                                    (FIELD_ID, IEMP.IDMETEMP, 1, AD_METEMPGRUSUBGRUPK, IGRUFI.GRUPO, IGRUFI.TOTAL, IEMP.CODEMP, IGRUFI.FATUR, (IGRUFI.TOTAL / IGRUFI.TOTALZAO)*100 );
--                                    IDEMPGRUFIPK := 1;
--                                ELSE
--                                
--                                    SELECT MAX(IDEMPGRUFI) + 1
...
--                                END IF;
                            --EXISTE UM IF QUE FECHA ABAIXO...
    ------------------------------------------------------------------------------------------------------------------------------------------------------------          
    --INSERE OS VENDEDORES COM GRUPOS FILHOS
    ------------------------------------------------------------------------------------------------------------------------------------------------------------
                                FOR IGRUFIVEND IN (SELECT PRO.CODGRUPOPROD AS GRUPO
                                                     , CAB.CODEMP
                                                     , SUM( ((ITE.VLRTOT - ITE.VLRDESC - ITE.VLRREPRED + ITE.VLRSUBST + ITE.VLRIPI)  * VCA.INDITENSBRUTO) * CASE WHEN TOP.BONIFICACAO = 'S' THEN 0 ELSE 1 END * TOP.GOLDEV ) AS TOTAL
                                                     , CAB.CODVEND
                                                     , FI.DATA AS FATUR
                                                     , (SELECT SUM( ((ITE.VLRTOT - ITE.VLRDESC - ITE.VLRREPRED + ITE.VLRSUBST + ITE.VLRIPI)  * VCA.INDITENSBRUTO) * CASE WHEN TOP.BONIFICACAO = 'S' THEN 0 ELSE 1 END * TOP.GOLDEV ) AS TOTAL
                                                        FROM AD_METEMPGRUSUBGRU FI INNER JOIN TGFCAB CAB ON (FI.CODEMP = CAB.CODEMP)
                                                         ...
                                                         WHERE FI.ID = FIELD_ID
                                                           AND FI.IDMETEMP = IEMP.IDMETEMP
                                                           AND FI.IDGRUMETDIA = IGRUFI.IDGRUMETDIA
                                                           AND FI.IDEMPGRUFI = AD_METEMPGRUSUBGRUPK
                                                           AND TRUNC(CAB.DTFATUR) = IGRUFI.FATUR
                                                           AND PRO.CODGRUPOPROD = FI.CODGRUPOPROD
                                                           AND CAB.CODTIPOPER IN (SELECT REGEXP_SUBSTR(REPLACE(PCODTIPOPER, ' ',''),'[^,]+', 1, LEVEL) AS RESULTADO FROM DUAL CONNECT BY REGEXP_SUBSTR(REPLACE(PCODTIPOPER, ' ',''), '[^,]+', 1, LEVEL) IS NOT NULL)
                                                           AND CAB.STATUSNFE= 'A') AS TOTALZAO
                                                 FROM AD_METEMPGRUSUBGRU FI INNER JOIN TGFCAB CAB ON (FI.CODEMP = CAB.CODEMP)
                                                                           INNER JOIN TGFITE ITE ON (CAB.NUNOTA = ITE.NUNOTA)
                                                                           INNER JOIN TGFPRO PRO ON (ITE.CODPROD = PRO.CODPROD)
                                                                           INNER JOIN VGFCAB VCA ON (CAB.NUNOTA = VCA.NUNOTA)
                                                 ...
                                                 WHERE FI.ID = FIELD_ID
                                                   ...
                                                   AND CAB.CODTIPOPER IN (SELECT REGEXP_SUBSTR(REPLACE(PCODTIPOPER, ' ',''),'[^,]+', 1, LEVEL) AS RESULTADO FROM DUAL CONNECT BY REGEXP_SUBSTR(REPLACE(PCODTIPOPER, ' ',''), '[^,]+', 1, LEVEL) IS NOT NULL)
                                                   AND CAB.STATUSNFE= 'A'
                                                 GROUP BY PRO.CODGRUPOPROD, CAB.CODVEND, FI.DATA, CAB.CODEMP)
                                LOOP
                                    SELECT COUNT(*)
                                    INTO AD_METEMPGRUSUBGRUVENPK
                                    FROM AD_METEMPGRUSUBGRUVEN AD
                        ...
                        END IF; --SE O GRUPO NÃO FOR ANALITICO   
